...
@section('title', __('pages.vps.meta_title') . ' — ' . config('site.short_name'))
@section('meta_description', __('pages.vps.meta_description'))

@php
    use App\Support\HostingPricing;

    $highlights = __('pages.vps.highlights');
    if (! is_array($highlights)) {
        $highlights = [];
    }
    $steps = __('pages.vps.steps');
    if (! is_array($steps)) {
        $steps = [];
    }
    $features = __('pages.vps.features');
    if (! is_array($features)) {
        $features = [];
    }
    $fitVps = __('pages.vps.fit_vps');
    if (! is_array($fitVps)) {
        $fitVps = [];
    }
    $fitShared = __('pages.vps.fit_shared');
    if (! is_array($fitShared)) {
        $fitShared = [];
    }
    $faqItems = __('pages.vps.faq_items');
    if (! is_array($faqItems)) {
        $faqItems = [];
    }

    $vpsSpecs = collect(config('site.hosting_plans.vps.specifications', []))
        ->map(function (array $spec) {
            $key = (string) ($spec['key'] ?? '');
            $usd = HostingPricing::monthlyNgnForSpec('vps', $key);
            if ($usd <= 0) {
...
@section('content')
            ...
            <div class="grid items-center gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                <div>
                    ...
                    <div class="mt-8 flex flex-wrap items-center gap-3">
                        <a
                            href="{{ route('hosting.specifications', ['plan' => 'vps']) }}"
                        ...
                        >
                            ...
                            <span data-action-label>{{ __('pages.vps.cta') }}</span>
                            <span class="hidden" data-action-loading-label>{{ __('account.processing') }}</span>
                        </a>
                        <a href="#vps-plans" class="inline-flex items-center gap-2 text-sm font-semibold text-white/90 transition hover:text-white">
                            <x-ui.icons.arrow-down class="size-4" />
                            {{ __('pages.vps.plans_title') }}
                        </a>
                    </div>
                </div>
                <div class="dev-cutout-hero hidden lg:flex" aria-hidden="true">
                    <picture>
                        <source srcset="{{ asset('images/hosting/vps-hero.webp') }}" type="image/webp">
                        <img
                            src="{{ asset('images/hosting/vps-hero.png') }}"
                            alt=""
                            width="480"
                            height="480"
    ...
    <section class="section-band border-t border-border">
        <div class="container-page py-16 sm:py-20">
            <div class="mx-auto mb-10 max-w-2xl text-center">
                <p class="section-label mb-3">{{ __('pages.vps.intro_eyebrow') }}</p>
                <h2 class="heading">{{ __('pages.vps.intro_title') }}</h2>
                <p class="lede mx-auto mt-3">{{ __('pages.vps.intro_lede') }}</p>
            </div>
    ...
    <section id="vps-plans" class="border-t border-border bg-blush-soft/40 scroll-mt-28">
        <div class="container-page py-16 sm:py-20">
            <div class="mb-10 max-w-2xl">
                <p class="section-label mb-3">{{ __('pages.vps.plans_eyebrow') }}</p>
                <h2 class="heading">{{ __('pages.vps.plans_title') }}</h2>
                <p class="lede mt-3">{{ __('pages.vps.plans_lede') }}</p>
            </div>

            @if (count($vpsSpecs) > 0)
                <div class="hosting-plan-grid">
                    @foreach ($vpsSpecs as $index => $spec)
                        <article @class(['hosting-plan-card', 'hosting-plan-card-featured' => $index === 1])>
                            ...
                            <p class="mt-4 text-sm font-semibold">
                                <span class="text-xs font-medium uppercase tracking-wide {{ $index === 1 ? 'text-white/60' : 'text-on-blush/50' }}">{{ __('pages.vps.plans_from') }}</span>
                            ...
                            <dl @class(['hosting-plan-specs', 'hosting-plan-specs-on-dark' => $index === 1])>
                                <div>
                                    <dt>{{ __('pages.vps.plans_cpu') }}</dt>
                                    <dd>{{ $spec['cpu'] }}</dd>
                                </div>
                                <div>
                                    <dt>{{ __('pages.vps.plans_ram') }}</dt>
                                    <dd>{{ $spec['ram'] }}</dd>
                                </div>
                                <div>
                                    <dt>{{ __('pages.vps.plans_storage') }}</dt>
                                    <dd>{{ $spec['storage'] }}</dd>
                                </div>
                            </dl>
                            ...
                            @endif
                            <a
                                href="{{ route('hosting.specifications', ['plan' => 'vps', 'spec' => $spec['key']]) }}"
                                @class([
                            ...
                            >
                                ...
                                <span data-action-label>{{ __('pages.vps.plans_select') }}</span>
                                <span class="hidden" data-action-loading-label>{{ __('account.processing') }}</span>
                            </a>
                        </article>
                    @endforeach
    ...
    <section id="vps-benefits" class="section-band border-t border-border scroll-mt-28">
        <div class="container-page py-16 sm:py-20">
            <div class="mb-10 max-w-2xl">
                <p class="section-label mb-3">{{ __('pages.vps.features_eyebrow') }}</p>
                <h2 class="heading">{{ __('pages.vps.features_title') }}</h2>
                <p class="lede mt-3">{{ __('pages.vps.features_lede') }}</p>
            </div>
    ...
    <section class="border-t border-border bg-blush-soft/40">
        <div class="container-page py-16 sm:py-20">
            <div class="mb-10 max-w-2xl">
                <p class="section-label mb-3">{{ __('pages.vps.fit_eyebrow') }}</p>
                <h2 class="heading">{{ __('pages.vps.fit_title') }}</h2>
                <p class="lede mt-3">{{ __('pages.vps.fit_lede') }}</p>
            </div>
            <div class="grid gap-6 lg:grid-cols-2">
                <article class="rounded-3xl border border-border bg-white p-7 sm:p-8">
                    <h3 class="text-lg font-bold text-black">{{ __('pages.vps.fit_vps_title') }}</h3>
                ...
                <article class="rounded-3xl border border-border bg-white p-7 sm:p-8">
                    <h3 class="text-lg font-bold text-black">{{ __('pages.vps.fit_shared_title') }}</h3>
                    <ul class="check-list mt-5 space-y-3">
                    ...
                    <a href="{{ route('cloud-hosting') }}" class="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-rose hover:underline">
                        <span>{{ __('pages.vps.fit_shared_cta') }}</span>
        ...
        <section class="border-t border-border bg-white">
            <div class="container-page py-16 sm:py-20">
                <div class="mx-auto max-w-3xl">
                    <h2 class="heading mb-8 text-center">{{ __('pages.vps.faq_title') }}</h2>
                    <x-ui.accordion>
    ...
    <section class="border-t border-border bg-blush-soft/40">
        <div class="container-page py-14 sm:py-16">
            <h2 class="heading mb-3">{{ __('pages.vps.help_title') }}</h2>
            <p class="lede mb-6">{{ __('pages.vps.help_lede') }}</p>
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('hosting.specifications', ['plan' => 'vps']) }}" class="btn btn-primary">
                    <x-ui.icons.arrow-up-right class="size-4" />
                    <span>{{ __('pages.vps.cta') }}</span>
                </a>
